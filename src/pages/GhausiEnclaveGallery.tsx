...
const images = [
  ...
  // We can add more images here if provided, for now reuse the ones from the gallery
  "https://res.cloudinary.com/dbshx9xvx/image/upload/v1783685417/Architectural_visualization_luxu__2K_202607101739_sc5moi.jpg" // Using the hero image as well
];
...
export function GhausiEnclaveGallery() {
  ...
  const breadcrumbSchema = JSON.stringify({
    ...
    "itemListElement": [{
      ...
      "@type": "ListItem",
      "position": 2,
      "name": "Ghausi Enclave Project Gallery",
      "item": "https://homeheart.in/projects/ghausi-enclave/gallery"
    }]
  });
  ...
  return (
    ...
    <SEO title="Ghausi Enclave Project Gallery | Home Heart Construction" description="Explore the luxury amenities, master plan, and actual photos of Ghausi Enclave, a premium residential project by Home Heart Construction in Gaya." schema={[breadcrumbSchema]} />
      <div className="max-w-[1400px] mx-auto px-6 md:px-8 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          >
            Discover architecture, interiors, amenities, layouts, premium living spaces and every important detail of the project.
          </motion.p>
        </div>
        ...
        >
          {images.map((src, idx) => (
            ...
            >
              <img 
                src={src} 
                alt={`Ghausi Enclave architectural visualization ${idx + 1}`}
                loading="lazy"
                decoding="async"
            ...
            <div className="relative max-w-[90vw] max-h-[90vh] flex items-center justify-center w-full h-full p-4 md:p-12" onClick={(e) => e.stopPropagation()} onDoubleClick={() => setScaleLevel(prev => prev === 1 ? 2 : 1)} onWheel={(e) => { e.stopPropagation(); if (e.deltaY < 0) setScaleLevel(prev => Math.min(prev + 0.5, 4)); else setScaleLevel(prev => Math.max(prev - 0.5, 1)); }} style={{ overflow: scaleLevel > 1 ? "auto" : "hidden" }}>
              <AnimatePresence mode="wait">
                <motion.img 
                  ...
                  alt={`Ghausi Enclave architectural visualization ${selectedIndex + 1}`}
