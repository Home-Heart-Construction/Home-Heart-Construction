  ...
  const renderComparisonContent = (expanded: boolean) => (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <img 
          src="/transformation/mazhari 1.jpeg" 
          alt="After Construction - Luxury Apartment" 
          className="w-full h-full object-cover"
        loading="lazy" decoding="async" />
        <div className="absolute bottom-2 right-2 sm:bottom-6 sm:right-6 bg-black/50 backdrop-blur-md px-2 py-1 sm:px-4 sm:py-2 rounded-full border border-white/10 z-10">
      ...
      >
        <img 
          src="/transformation/mazhari 2.jpeg" 
          alt="Before Construction - Structure" 
          className="w-full h-full object-cover"
        loading="lazy" decoding="async" />
        <div className="absolute bottom-2 left-2 sm:bottom-6 sm:left-6 bg-black/50 backdrop-blur-md px-2 py-1 sm:px-4 sm:py-2 rounded-full border border-white/10 z-10">
      ...
      {/* Slider Handle */}
      <div 
        className={`absolute top-1/2 -translate-y-1/2 -ml-4 sm:-ml-6 z-40 w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-white/10 backdrop-blur-xl border border-white/30 flex items-center justify-center cursor-ew-resize shadow-lg transition-transform duration-300 hover:scale-110 hover:border-gold hover:bg-white/20 hover:shadow-[0_0_20px_rgba(200,164,106,0.3)] ${isDragging ? 'scale-110 border-gold bg-white/20 shadow-[0_0_20px_rgba(200,164,106,0.3)]' : ''}`}
        style={{ left: `${sliderPosition}%` }}
      >
      ...
      <div className="absolute top-2 right-2 sm:top-6 sm:right-6 z-50 flex items-center group/expand">
        {!expanded && (
